...
public abstract class MusicChartParser {
   ...
   // public abstract void songDetailDataParsing(int rank, JSONArray chartListData, Component parentComponent);
    ...
    public JSONObject getSongData() { // 노래 한 곡에 대한 상세 정보를 담는 JSONObject인 songDetailInfo를 반환하는 메소드
        ...
        if (_songCount == 100) { // 차트 100곡에 대한 파싱을 했다면
    ...
    public JSONObject getSongData(int rank) { // 노래 한 곡에 대한 상세 정보 또는 차트 100위의 노래 1곡에 대한 정보를 반환하는 메소드
        ...
        if (_songCount == 1)        // 노래 한 곡에 대한 상세 정보 파싱이 이루어졌다면 상세 정보 반환(상세 정보는 순위에 상관이 없음)
            return _songDetailInfo;
    ...
    public JSONObject getSongData(String title) { // 노래 한 곡에 대한 상세 정보 또는 차트 100위의 노래 1곡에 대한 정보를 반환하는 메소드
        ...
        for (int i = 0; i < _songCount; i++) { // 차트 100곡에 대한 파싱이 이루어졌다면 JSONArray에 있는 노래들 중 title 제목에 맞는 원소를 반환
    ...
    *//*
    public int getRank(String title) { // 노래 제목을 통해 해당 노래의 순위를 반환하는 메소드
        ...
        if (_songCount == 1) { // 노래 한 곡에 대한 상세 정보 파싱이 이루어졌다면 -1 반환(상세 정보에는 순위가 없기 때문)
            System.out.println("getRank(String title) :" + _isOnlyChartParse);
            return -1;
        }

        for (int i = 0; i < _songCount; i++) { // 차트 100곡에 대한 파싱이 이루어졌다면 JSONArray에 있는 노래들 중 title 제목에 맞는 원소의 순위를 반환
            if (((JSONObject) _chartList.get(i)).get("title") == title)
    ...
    */
    ...
    public String getArtistName(String title) { // 노래 제목을 통해 해당 노래의 가수 이름을 반환하는 메소드
        ...
        if (_songCount == 1) { // 노래 한곡에 대한 상세 파싱이 이루어졌다면
            System.out.println("getArtistName(String title) : " + _isOnlyChartParse);
            return null;
        }

        for (int i = 0; i < _songCount; i++) { // 차트 100곡에 대한 파싱이 이루어졌다면 JSONArray에 있는 노래들 중 title 제목에 맞는 원소의 가수 이름을 반환
            if (((JSONObject) _chartList.get(i)).get("title") == title)
    ...
    */
    ...
    public String getAlbumName(String title) { // 노래 제목을 통해 해당 노래의 앨범 이름을 반환하는 메소드
        ...
        for (int i = 0; i < _songCount; i++) { // 차트 100곡에 대한 파싱이 이루어졌다면 JSONArray에 있는 노래들 중 title 제목에 맞는 원소의 앨범 이름을 반환
            if (((JSONObject) _chartList.get(i)).get("title") == title)
    ...
    */
    ...
    public String getImageUrl(String title) { // 노래 제목을 통해 해당 노래의 이미지 url을 반환하는 메소드
        ...
        if (_songCount == 1) // 노래 한 곡에 대한 상세 파싱이 이루어졌다면
            return _songDetailInfo.get("imageUrl").toString(); // 노래 한 곡에 대한 상세 정보 파싱이 이루어졌다면 그 곡의 큰 이미지 url을 반환(상세 정보는 제목과 상관이 없음)

        for (int i = 0; i < _songCount; i++) { // 차트 100곡에 대한 파싱이 이루어졌다면 JSONArray에 있는 노래들 중 title 제목에 맞는 원소의 작은 이미지 url을 반환
            if (((JSONObject) _chartList.get(i)).get("title") == title)
    ...
    public String getImageUrl(JSONObject jObj) { // JSONArray의 원소 중 하나를 이용하여 해당 노래의 좋아요 개수를 반환하는 메소드
        ...
        if (_songCount == 1) { // 노래 한 곡에 대한 상세 파싱이 이루어졌다면 큰 이미지 url 반환
